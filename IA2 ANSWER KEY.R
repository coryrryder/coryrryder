#############################################
####### INDIVIDUAL ASSIGNMENT 2 ANSWERS #####
#############################################
...
#4. Read data file
my.df = read.csv("zillow22.csv", header=TRUE, sep=',', row.names="name", stringsAsFactors = TRUE)
...
#6. Make  zipcode factor. # The model will run incorrectly, if zipcode is still int.
my.df$zipcode <- factor(my.df$zipcode)

#7. Calculate the age of the house variable ("age") and add the age variable to the dataframe
my.df$age <- 2022 - my.df$year

#8. Print summary of age
summary(my.df$age) #Q01-1

#9. Remove features we won't need
my.df<-my.df[,-1] #section
my.df<-my.df[,-1] #team
my.df<-my.df[,-1] #international
my.df<-my.df[,-7] #zestimate
my.df<-my.df[,-8] #assessment
my.df<-my.df[,-13] #willingtopay
my.df<-my.df[,-3] #year
...
#8. **Print** LaylaL's residual (calculated as Y - Y_hat) **(#Q2-5)**
my.df["LaylaL", "price"]-lasso.pred["LaylaL",] #Q02-5
